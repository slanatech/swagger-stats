...
var defaultIndexTemplate = require('../schema/elasticsearch/api_index_template.json');
...
swsElasticEmitter.prototype.initTemplate = function(rrr) {
    ...
    // merge custom schema in to default one
    // Object.assign is not doing a deep copy so it overrides everything in same key
    // it might be simpler to provide full schema not just a delta on top of default
    // one so that there is no need to care about merging jsons properly
    // right now, it is actually overriding default template but once i refresed indexes
    // on elk it works and newly added properties are indexed.
    if (that.options.schema) {
        indexTemplate = Object.assign({}, defaultIndexTemplate, that.options.schema);
    }
