...
swsProcessor.prototype.collectRequestResponseData = function (res) {
    ...
    if ("swaggerstats" in req) {
        ...
        if( req.swaggerstats.api_path != ''){
            ...
            // If this is call to API not defined in swagger spec, fall back to route_path or originalUrl
            reqresdata.api.path = (req.swaggerstats.route_path != '' ? req.swaggerstats.route_path : req.originalUrl);
        }
...
swsProcessor.prototype.processResponse = function (res) {
    ...
    // If api_path not found, fall back to either route_path, or originalUrl
    if(api_path!='') {
        req.swaggerstats.api_path = api_path;
    }else{
        req.swaggerstats.api_path = (route_path!=''?route_path:req.originalUrl);
    }
    ...
    if(swsUtil.isError(res.statusCode)){
        this.lastErrors.addError(reqresdata);
    }
