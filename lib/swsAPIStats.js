...
var swsHistogram = require('./swsHistogram');
...
swsAPIStats.prototype.writeMetrics = function (stream) {

    stream.write(swsUtil.swsMetricStart(swsUtil.swsMetrics.api_request_total.name));

    for(var path in this.apidetails){
        for( var method in this.apidetails[path] ){
            var apiOpDetails = this.getApiOpDetails(path,method);
            for( var code in apiOpDetails.code ) {
                stream.write(swsUtil.swsMetricValue(
                    swsUtil.swsMetrics.api_request_total.name,
                    apiOpDetails.code[code].count,
    ...
    // Durations
    stream.write(swsUtil.swsMetricStart(swsUtil.swsMetrics.api_request_duration_milliseconds.name));
    for(var path in this.apidetails){
        for( var method in this.apidetails[path] ){
            var apiOpDetails = this.getApiOpDetails(path,method);

            var hist = apiOpDetails.duration;
            for(var i=0;i<hist.buckets.length;i++){
                stream.write(swsUtil.swsMetricValue(
                    swsUtil.swsMetrics.api_request_duration_milliseconds.name,
                    hist.cumulative_values[i],
                    { method:method, path:path, le:hist.buckets[i] },
                    '_bucket'
                ));
            }
            stream.write(swsUtil.swsMetricValue(
    ...
    // Request size
    stream.write(swsUtil.swsMetricStart(swsUtil.swsMetrics.api_request_size_bytes.name));
    for(var path in this.apidetails){
        for( var method in this.apidetails[path] ){
            var apiOpDetails = this.getApiOpDetails(path,method);
            var hist = apiOpDetails.req_size;
            for(var i=0;i<hist.buckets.length;i++){
                stream.write(swsUtil.swsMetricValue(
                    swsUtil.swsMetrics.api_request_size_bytes.name,
                    hist.cumulative_values[i],
                    { method:method, path:path, le:hist.buckets[i] },
                    '_bucket'
                ));
    ...
    // Response size
    stream.write(swsUtil.swsMetricStart(swsUtil.swsMetrics.api_response_size_bytes.name));
    for(var path in this.apidetails){
        for( var method in this.apidetails[path] ){
            var apiOpDetails = this.getApiOpDetails(path,method);
            var hist = apiOpDetails.res_size;
            for(var i=0;i<hist.buckets.length;i++){
                stream.write(swsUtil.swsMetricValue(
                    swsUtil.swsMetrics.api_response_size_bytes.name,
                    hist.cumulative_values[i],
                    { method:method, path:path, le:hist.buckets[i] },
                    '_bucket'
                ));
...
swsAPIStats.prototype.getApiOpDetails = function(path,method) {
    ...
    if(!(method in this.apidetails[path]) ) this.apidetails[path][method] = {
        duration: new swsHistogram(this.durationBuckets),       // Request duration histogram
        req_size: new swsHistogram(this.requestSizeBuckets),    // Request size histogram
        res_size: new swsHistogram(this.responseSizeBuckets),   // Response size histogram
