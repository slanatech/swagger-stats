...
function swsAPIStats() {
    ...
    // Test histogram
    this.histogram = new swsHistogram([1,10,100]);
...
swsAPIStats.prototype.getMetrics = function () {
    ...
    metrics += '# HELP api_http_requests_total Total number of API HTTP requests made\n';
    metrics += '# TYPE api_http_requests_total counter\n';

    for(var path in this.apidefs){
        for( var method in this.apidefs[path] ){

            var apiOpStats = this.getAPIOpStats(path,method);

            var metric = util.format('api_http_requests_total{method="%s",path="%s"} %s\n', method, path, apiOpStats.requests );

            // TODO Split by response class
...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    // TEMP
    this.histogram.countValue(req.sws.duration);
