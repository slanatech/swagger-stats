...
function swsAPIStats() {
    ...
    // rest of Prometheus metrics are initialized in initialize() after user options have been processed
}
...
swsAPIStats.prototype.initialize = function(swsOptions) {
    ...
    if(!this.promClientMetrics.api_request_duration_milliseconds || 'durationBuckets' in swsOptions) {
        // promClient is global, so metric needs to be re-initialized here
        promClient.register.removeSingleMetric(swsUtil.swsMetrics.api_request_duration_milliseconds.name);
    ...
    if(!this.promClientMetrics.api_request_size_bytes || 'requestSizeBuckets' in swsOptions) {
        promClient.register.removeSingleMetric(swsUtil.swsMetrics.api_request_size_bytes.name);
    ...
    if(!this.promClientMetrics.api_response_size_bytes || 'responseSizeBuckets' in swsOptions) {
        promClient.register.removeSingleMetric(swsUtil.swsMetrics.api_response_size_bytes.name);
