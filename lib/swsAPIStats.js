...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    var req = res.req;
    var codeclass = swsUtil.getStatusCodeClass(res.sws ? res.response.status:res.statusCode);

    // Only intersted in updating stats here
    var apiOpStats = this.getAPIOpStats(res.sws ? res.sws.api_path: req.sws.api_path,req.method);
    ...
    if (!('match' in (res.sws ? res.sws :req.sws)) || !(res.sws?res.sws.match:req.sws.match)) {
        apiOpStats.countRequest(res.sws?res.sws.req_clength: req.sws.req_clength);
    }

    // In all cases, count response here
    apiOpStats.countResponse(
        res.sws ? res.response.status : res.statusCode,
        codeclass,
        res.sws ? res.sws.duration : req.sws.duration,
        res.sws ? res.sws.res_clength : req.sws.res_clength
        );

    // Count metrics
    var apiOpDetails = this.getApiOpDetails(
        res.sws ? res.sws.api_path : req.sws.api_path,
        req.method
    );
    ...
    if (!((res.sws ? res.response.status : res.statusCode)in apiOpDetails.code)){
        apiOpDetails.code[res.sws ? res.response.status : res.statusCode] = { count:0 };
    }

    apiOpDetails.code[res.sws ? res.response.status : res.statusCode].count++;
    apiOpDetails.duration.countValue(res.sws ? res.sws.duration : req.sws.duration);
    apiOpDetails.req_size.countValue(res.sws ? res.sws.req_clength : req.sws.req_clength);
    apiOpDetails.res_size.countValue(res.sws ? res.sws.res_clength : req.sws.res_clength);

    // update Prometheus metrics
    this.promClientMetrics.api_request_total.labels(
        req.method,
        res.sws ? res.sws.api_path : req.sws.api_path,
        res.sws ? res.response.status : res.statusCode).inc();

    this.promClientMetrics.api_request_duration_milliseconds.labels(
        req.method,
        res.sws ? res.sws.api_path : req.sws.api_path,
        res.sws ? res.response.status : res.statusCode).observe(res.sws ? res.sws.duration : req.sws.duration);
    this.promClientMetrics.api_request_size_bytes.labels(
        req.method,
        res.sws ? res.sws.api_path : req.sws.api_path,
        res.sws ? res.response.status : res.statusCode).observe(res.sws ? res.sws.req_clength : req.sws.req_clength);
    this.promClientMetrics.api_response_size_bytes.labels(
        req.method,
        res.sws ? res.sws.api_path : req.sws.api_path,
        res.sws ? res.response.status : res.statusCode).observe(res.sws ? res.sws.res_clength : req.sws.res_clength);
