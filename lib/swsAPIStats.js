...
swsAPIStats.prototype.getMetrics = function () {

    var metrics = '';

    metrics += swsUtil.swsMetricStart(swsUtil.swsMetrics.api_requests_total.name);
    //metrics += swsUtil.swsMetricValue(swsUtil.swsMetrics.api_all_server_error_total.name,this.all.server_error);
    ...
    for(var path in this.apidetails){
        for( var method in this.apidetails[path] ){
            ...
            for( var code in apiOpDetails.code ) {
                //var metric = util.format('api_requests_total{method="%s",path="%s",code="%s"} %s\n', method, path, code, apiOpDetails.code[code].count );
                metrics += swsUtil.swsMetricValue(
                    swsUtil.swsMetrics.api_requests_total.name,
...
swsAPIStats.prototype.getApiOpDetails = function(path,method) {
    ...
    if(!(method in this.apidetails[path]) ) this.apidetails[path][method] = { code: {} };
...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    if(!(res.statusCode in apiOpDetails.code)){
        apiOpDetails.code[res.statusCode] = {
            count:     0,
    ...
    var metrics = apiOpDetails.code[res.statusCode];
    metrics.count++;
    metrics.duration.countValue(req.sws.duration);
    metrics.req_size.countValue(req.sws.req_clength);
    metrics.res_size.countValue(req.sws.res_clength);

    // TODO Consider adding metrics by code class
