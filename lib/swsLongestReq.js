...
swsLongestRequests.prototype.placeReqResData = function(rrr) {
    ...
    if( duration >= this.highest ){
        ...
        debug('%d >= highest: %d',duration,this.longest_requests.length);
        return;
    }
    ...
    if( duration <= this.lowest ) {
        ...
        debug('%d <= lowest(%d): %d',duration,this.lowest,this.longest_requests.length);
        return;
    }
    ...
    debug('%d - Lookup: idx=%d',duration,idx);
...
swsLongestRequests.prototype.processReqResData = function(rrr) {
    ...
    if( !this.full ){
        ...
        debug('Not full: %d',this.longest_requests.length);
