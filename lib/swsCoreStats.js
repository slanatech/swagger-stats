...
swsCoreStats.prototype.countResponse = function(res) {
    ...
    if ("_contentLength" in res && res['_contentLength'] !== null) {
        ...
        if (res.getHeader && res.getHeader('content-length') !== null) {
            ...
            if (res.response['length'] !== null) {
                resContentLength = parseInt(res.response['length'] || 0);
            }
        }
    }
    ...
    if ("sws" in req) {
        ...
        startts = res.sws.startts;
        timelineid = res.sws.timelineid;
        var endts = Date.now();
        res['sws'].endts = endts;
        duration = endts - startts;
        res['sws'].duration = duration;
        res['sws'].res_clength = resContentLength;
        path = res['sws'].api_path;
    }

    // Determine status code type
    var codeclass = swsUtil.getStatusCodeClass(res.statusCode || res.response.status);

    // update counts for all requests
    this.all.countResponse(res.statusCode || res.response.status, codeclass, duration, resContentLength);

    // Update method-specific stats
    var method = req.method;
    if (method in this.method) {
        var mstat = this.method[method];
        mstat.countResponse(res.statusCode || res.response.status, codeclass, duration, resContentLength);
    }
