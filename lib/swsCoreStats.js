...
function swsCoreStats() {
    ...
    // System statistics
    this.sys = {
        memory: {},     // as reported by process.memoryUsage()
        cpu: 0
...
swsCoreStats.prototype.calculateSystemStats = function(ts,totalElapsedSec) {
    ...
    var elapUsage = process.cpuUsage(this.startUsage); // microseconds
    var cpuPercent = (100 * (elapUsage.user + elapUsage.system) / elapsedmcs );

    //console.log('cpu: '+ cpuPercent.toFixed(2) + '  elapsed: ' + elapsedns + ', usage ' + JSON.stringify(elapUsage));
