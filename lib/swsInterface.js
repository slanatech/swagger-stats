...
module.exports = {
    ...
    getMiddlewareKoa: function (app,options) {
        processOptions(options);
        processor = new swsProcessor();
        ...
        if(app){
            app.on('error', (err, ctx) => {
                if (ctx) {
                    ...
                    handleResponseKoa(ctx)
                }
            });
        }
        return  async function(ctx, next) {
            if (ctx.url.startsWith(pathStats)){
                return processGetStatsKoa(ctx);
            } else if (ctx.url.startsWith(pathMetrics)) {
                return processGetMetricsKoa(ctx);
...
function handleRequestKoa(ctx) {
    try {
        processor.processorRequestKoa(ctx);
    } catch (error) {
        console.log(error)
    }
}

function handleResponseKoa(ctx) {
    try {
        processor.processorResponseKoa(ctx);
    } catch (error) {
        console.log(error)
    }
}
...
async function processGetStatsKoa(ctx) {
    ...
    if (!cluster.isMaster) {
        ...
        content = processor.getStats(query);
    }
    ctx.body = content;
}
