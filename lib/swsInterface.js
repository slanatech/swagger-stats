...
// Parameter fields defines which stat fields to return
function processGetStats(req,res){

    var fields = [];    // Empty by default

    // Check if we have query parameter "fields"
    if(('query' in req) && ('fields' in req.query) ){
        var fieldsstr = req.query.fields;
        fields = fieldsstr.split(',');
    }

    res.status(200).json(processor.getStats(fields));
}
    ...
    getMiddleware: function () {
        ...
        return function trackingMiddleware(req, res, next) {
            ...
            if(req.url == '/swagger-stats/data') {
                ...
                return;
            }else if(req.url.startsWith('/swagger-stats/stats')){
                processGetStats(req,res);
                return;
                // TODO Support /swagger-stats/apiopstats?path=path&method=method
