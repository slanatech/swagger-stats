...
function processExpireSession() {
    swsAuth.expireSessionIDs();
}
...
function processGetStats(req, res) {
    swsAuth.processAuth(req, res).then(function (authResult) {
        if (!authResult) {
            return;
        }
        res.statusCode = 200;
...
function expressMiddleware(options) {
    ...
    if (swsSettings.authentication) {
        setInterval(processExpireSession, 500);
    }
...
function fastifyPlugin(fastify, opts, done) {
    ...
    fastify.addHook('onResponse', (request, reply, done) => {
        ...
        if (
            'context' in reply &&
            'config' in reply.context &&
            'url' in reply.context.config
        ) {
